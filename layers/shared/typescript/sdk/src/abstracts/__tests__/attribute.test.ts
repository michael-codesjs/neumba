...
describe("Attribute", () => {
	...
	test("validator on .set", () => {
		...
		const attribute = new Attribute<string, false>({ value: validValue, validate: mockValidator });

		attribute.set(validValue);
		
		expect(mockValidator.mock.calls).toHaveLength(1);
		expect(() => attribute.set("other value")).toThrowError(new InvalidAttributeValue());
	...
	test(".isUpdateable", () => {
		...
		castedAttribute.modified = new Date(modified.valueOf() + chance.integer({ min: 1, max: 1000 }));
