...
const inputMapper = async (input: ESTATE_CREATED_DOMAIN_EVENT): Promise<void> => {
  const { payload } = input;
  await publishEstateUpdatedEvent(payload);
}

/** 'publishEstateCreated' lambda function handler. */
export const handler: CommonInputHandler<ESTATE_CREATED_DOMAIN_EVENT, void> = withCommonInput(inputMapper, { singular: true as true });
