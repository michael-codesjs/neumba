...
describe("Attributes", () => {
	...
	const getRandomAttributeDefinitions = () => {
		...
		const randomAttributeKeys = (
			Array(chance.integer({ min: 5, max: 100 }))
				.fill(null)
				.map(() => chance.word())
		);
	...
	const getRandomAttributeValues = <T extends string>(attributes: Array<T>): Record<T, any> => {
		return attributes.reduce((cumulative, current) => ({
			...cumulative,
	...
	type GetRandomAttributesReturnType = () => {
		attributes: Attributes<CommonAttributes & Record<string, AttributeSchema<any, boolean>>>,
	...
	const getRandomAttributes: GetRandomAttributesReturnType = () => {
		...
		const attributes = new Attributes<CommonAttributes & Record<string, AttributeSchema<any, boolean>>>(definitions);
	...
	const COMMON_ATTRIBUTES = ["entityType", "id", "creatorType", "creator", "created", "modified", "discontinued"];

	test("instanciates Common attributes on Attributes instaciation", () => {

		const attributes = new Attributes();
		const instanciatedAttributes = (attributes as any).Attributes as Record<keyof CommonAttributes, Attribute<any, boolean>>;

		expect(Object.keys(instanciatedAttributes))
			.toEqual(expect.arrayContaining(COMMON_ATTRIBUTES));

		Object.entries(instanciatedAttributes)
			.forEach(value => {
	...
	test("instanciates T attributes on Attributes instanciation", () => {
		...
		const instanciatedAttributes = (attributes as any).Attributes as Record<string, Attribute<string, boolean>>;
	...
	test(".parse(common)", () => {
		...
		const _Attributes = (attributes as any).Attributes as Record<keyof CommonAttributes, Attribute<string, boolean>>;

		Object.entries(commonAttributesValues)
			.forEach(([key, value]) => {
	...
	test(".parse(T)", () => {
		...
		attributes.parse({
			entityType: getRandomEntityType(), // is the only requireed attribute when parsing attributes
			...values
		});

		const _Attributes = (attributes as any).Attributes as Record<keyof typeof values, Attribute<string, boolean>>;
